...
export async function main(argv=process.argv.slice(2)){
 ...
 async function one(page,temperature,session){
  ...
  const comparison_ms=await timing(page,async()=>{
   // Measure the identical browser journey before any optional song/Back journey.
  ...
  await page.locator('#catalog-tab').click();await frames(page);
