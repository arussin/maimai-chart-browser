...
async function importReport(page,remember=true){await openImport(page);await page.getByLabel('Hosted Session Report',{exact:true}).check();await page.getByLabel('Hosted Session Report URL',{exact:true}).fill(manifest);await page.getByLabel('Remember this profile and refresh it when I return').setChecked(remember);await page.getByRole('button',{name:'Continue',exact:true}).click();await page.getByRole('button',{name:remember?'Import & remember':'Import once',exact:true}).click();await expect.poll(()=>page.evaluate(()=>maimaiPersonal.enabled())).toBe(true);if(remember)await expect.poll(async()=>!!(await saved(page)).active?.source).toBe(true);}
