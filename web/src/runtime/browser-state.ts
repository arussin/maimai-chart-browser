...
export function restoreSortRules(value: unknown): SortRule[] {
  ...
  const valid = value.filter(
    (row): row is SortRule =>
      object(row) &&
      allKeys.has(row.key as SortKey) &&
      [1, -1].includes(Number(row.direction)) &&
      typeof row.direction === 'number',
  );
  return valid.length
    ? valid.map(({ key, direction }) => ({ key, direction }))
    : [{ key: 'title', direction: 1 }];
}

export function effectiveSortRules(
  requested: readonly SortRule[],
...
export class BrowserState {
  ...
  setPatterns(values: unknown): void {
    this.patterns.clear();
    strings(values)
      .filter((value) => this.patternIds.has(value))
      .forEach((value) => this.patterns.add(value));
  }
  ...
  restoreFilters(value: unknown): void {
    ...
    strings(value.difficulties)
      .filter((item) => this.difficultyIds.has(item))
    ...
    this.level.high =
      maximum < this.level.low ? Math.max(this.level.low, this.levels.length - 1) : maximum;
  }
  ...
  restorePersonal(value: unknown): void {
    ...
    for (const key of ['recorded', 'min', 'max', 'rateMin', 'rateMax', 'lamp', 'sync'] as const) {
      this.personal[key] =
        typeof value[key] === 'string' && value[key].length <= 32 ? value[key] : '';
    }
    this.personal.grade.clear();
    strings(value.grade)
      .filter((grade) => grades.includes(grade))
      .forEach((grade) => this.personal.grade.add(grade));
  }
  ...
  restoreRegion(value: unknown): void {
    if (
      !object(value) ||
      !['', 'JP', 'INTL'].includes(String(value.availability)) ||
      typeof value.international !== 'boolean'
    )
      return;
      ...
      this.sortRules = [
        {
          key,
          direction:
            prior && this.sortRules[0] === prior ? (prior.direction === 1 ? -1 : 1) : direction,
        },
      ];
    }
  ...
  private restoreIdentities(value: Record<string, unknown>): void {
    ...
    strings(value.expandedRows)
      .filter((id) => this.chartIds.has(id))
      .forEach((id) => this.expandedRows.add(id));
    if (object(value.comparison)) {
      ...
      this.comparison.priority =
        value.comparison.priority === 'measurements' ? 'measurements' : 'patterns';
  ...
  private restorePresentation(value: Record<string, unknown>): BrowserSnapshot {
    ...
    const disclosures: BrowserSnapshot['disclosures'] = rows.filter(
      (row): row is [string, string] =>
        Array.isArray(row) &&
        disclosureIds.has(String(row[0])) &&
        ['true', 'false'].includes(String(row[1])),
    );
    for (const [id, expanded] of disclosures) this.disclosure(id).expanded = expanded === 'true';
    const history = strings(value.history).filter(
      (id) => id.startsWith('saved-pbs-') && this.chartIds.has(id.slice(10)),
    );
    this.history.clear();
    ...
    const auxiliary = object(value.auxiliary)
      ? {
          ...
          menus: (Array.isArray(value.auxiliary.menus) ? value.auxiliary.menus : []).filter(
            (row): row is [string, boolean] =>
              Array.isArray(row) && menuIds.has(String(row[0])) && typeof row[1] === 'boolean',
          ),
        }
      : undefined;
    const scroll: [number, number] =
      Array.isArray(value.scroll) &&
      value.scroll.length === 2 &&
      value.scroll.every((n) => typeof n === 'number' && Number.isFinite(n))
        ? [value.scroll[0], value.scroll[1]]
        : [0, 0];
    ...
    return {
      ...this.snapshot({
        auxiliary,
        scroll,
        locale: this.view!.localization.locale,
  ...
  restore(value: unknown): boolean {
    ...
    if (
      ...
      value.catalogHash !== this.catalogHash
    ) {
      return false;
    }
    const view = this.view;
    view.usage.suspend(() => {
      if (locales.has(value.locale as Locale))
        view.localization.setLocale(value.locale as Locale, { persist: false });
      this.search = text(value.search);
      this.genre = text(value.genre);
      this.format = ['all', 'STD', 'DX'].includes(String(value.format))
        ? String(value.format)
        : 'all';
      ...
      this.visible = Number.isSafeInteger(value.visible)
        ? Math.max(40, Math.min(this.chartIds.size, value.visible as number))
        : 40;
