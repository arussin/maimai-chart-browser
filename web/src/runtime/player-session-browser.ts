...
import type { HandoffTransfer, SessionEvent } from './player-session-contracts';
...
export function createSessionBrowser(key: (name: string) => string) {
  ...
  function connect(owner: ImportCoordinator, historyPort: Pick<HistoryPort, 'replaceState'>) {
    ...
    window.addEventListener('storage', (event) => {
      if (event.key === key('maimai-player-event'))
        try {
    ...
    const url = new URL(location.href),
      ...
      nonce = fragment.get('party-import');
    ...
    if (nonce && /^[a-f0-9-]{36}$/.test(nonce) && window.opener) {
      ...
      const opener: Window = window.opener,
        protocol = 'maimai-player-handoff/1';
      let connected = false,
        tries = 0;
      const announce = () => {
      ...
      window.addEventListener('message', function receive(event: MessageEvent<unknown>) {
        ...
        if (
          ...
          return;
        if (
          (event.origin !== 'null' && !/^https?:\/\//.test(event.origin)) ||
          connected ||
          event.source !== opener ||
        ...
        port.start();
        void owner.handoff(
          'offer' in value ? value.offer : undefined,
          'stale' in value && value.stale === true,
          handoffTransfer(port),
        );
      });
      announce();
    }
...
export function handoffTransfer(
  port: Pick<MessagePort, 'postMessage' | 'close' | 'onmessage'>,
): HandoffTransfer {
  return {
    send: (type) =>
      port.postMessage({
      ...
      new Promise<ArrayBuffer>((resolve, reject) => {
        ...
        const timeout = setTimeout(
          () =>
            finish(
              new Error(
                'The transfer was interrupted. Download the player file from the report and import it here.',
              ),
            ),
          45000,
        );
        port.onmessage = (event: MessageEvent<unknown>) => {
          ...
          if (value.type === 'data' && 'bytes' in value && value.bytes instanceof ArrayBuffer)
            finish(null, value.bytes);
          else if (value.type === 'error')
            finish(new Error('The latest data could not be transferred. Please try again.'));
        };
