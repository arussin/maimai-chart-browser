...
export function createChartComparison(ports) {
let comparison;
/* Choose any public chart pair or find similar measured demands, entirely locally. */
(()=>{'use strict';
const i18n=ports.localization||{text:(node,value)=>node.textContent=value,attribute:(node,key,value)=>node.setAttribute(key,value),option:(...args)=>new Option(...args),verbatim:value=>value,searchTerms:value=>value,parts:(values,separator)=>values.join(separator),message:(source,values)=>source.replace(/\{(\d+)\}/g,(_,n)=>values[n]),literal:(node,value)=>node.textContent=value};
...
function mount({data,eligibleIds}){
  ...
  const byId=new Map(data.catalog.map(c=>[c.chart_id,c])),state=ports.browserState.comparison,pickers={};
  const name=c=>ports.catalogQuery?.titleLabel(c,i18n.locale)||c.title.trim()||'〈Blank title〉';
  ...
  function identity(c){const box=make('div',undefined,'chosen-chart');box.append(ports.artwork.jacket(c),make('strong',i18n.verbatim(name(c))),make('p',i18n.parts([chartType(c),i18n.verbatim('Lv. '+(c.level||'?')),bpmText(c)],' · '),'muted'),make('p',i18n.verbatim(c.artist),'muted'),overview.chips(c),overview.graph(c,{compact:true}));const videoLink=ports.chartLinks.group(c);if(videoLink)box.append(videoLink);if(ports.personal)box.append(ports.personal.summary(c));return box;}
  function writeLink(){const url=new URL(location.href);for(const side of ['left','right']){if(state[side])url.searchParams.set(side,state[side]);else url.searchParams.delete(side);}ports.historyPort.replaceState(history.state,'',url);}
  function choose(side,id,write=true){
    id=ports.registry.resolve(data,id);
    if(!byId.has(id))return;
    if(side==='left'&&state.left!==id){matches=null;state.similar=false;}
    const deliberate=write&&!!(side==='right'?state.left:state.right)&&(side==='right'?state.left:state.right)!==id;if(deliberate)ports.usage?.emit('compare_requested');
    ports.browserState.chooseComparison(side,id);const picker=pickers[side];picker.input.value=name(byId.get(id));picker.close();picker.selection.replaceChildren(identity(byId.get(id)));
    render();if(deliberate)ports.usage?.emit('compare_loaded');if(write)writeLink();
  }
  for(const [side,title]of [['left','First chart'],['right','Second chart']]){
    ...
    function search(){
      ...
      const matchesSearch=ports.songSearch.query(input.value),query=input.value.normalize('NFKC').toLowerCase().trim();
    ...
    input.oninput=event=>{if(event.isComposing)return;ports.usage?.emit('search_used',undefined,'compare');if(side==='left'){matches=null;state.similar=false;}ports.browserState.chooseComparison(side,null);selection.replaceChildren();render();writeLink();search();};
  ...
  function find(scroll=true){if(!state.left||!byId.get(state.left)?.demand)return;state.similar=true;matches=getIndex().similar(state.left,{limit:8,eligibleIds:state.useFilters?eligibleIds():null,patternCompare:state.priority==='patterns'?overview.compare:null});render();if(scroll)el('similar-results').scrollIntoView({block:'start',behavior:'instant'});}
  el('similar-priority').onchange=()=>{state.priority=el('similar-priority').value;if(matches!==null){ports.usage?.emit('similar_requested');find();}};
  el('find-similar').onclick=()=>{ports.usage?.emit('similar_requested');const u=new URL(location.href);u.searchParams.set('similar','1');ports.historyPort.replaceState(history.state,'',u);find();};el('similar-use-filters').onchange=()=>{state.useFilters=el('similar-use-filters').checked;if(matches!==null){ports.usage?.emit('similar_requested');find();}};
  el('comparison-clear').onclick=()=>{ports.browserState.chooseComparison('left',null);ports.browserState.chooseComparison('right',null);matches=null;state.similar=false;for(const picker of Object.values(pickers)){picker.input.value='';picker.selection.replaceChildren();picker.close();}render();writeLink();pickers.left.input.focus();};
  function restore(){const params=new URLSearchParams(location.search);let missing=false;matches=null;state.similar=false;
    for(const side of ['left','right']){const id=ports.registry.resolve(data,params.get(side));ports.browserState.chooseComparison(side,null);pickers[side].input.value='';pickers[side].selection.replaceChildren();pickers[side].close();if(id){if(byId.has(id))choose(side,id,false);else missing=true;}}
  ...
  ports.historyPort.onTraversal(restore);restore();
  return{render,sync(){matches=null;el('similar-priority').value=state.priority;el('similar-use-filters').checked=state.useFilters;for(const side of ['left','right']){const chart=byId.get(state[side]);pickers[side].input.value=chart?name(chart):'';pickers[side].selection.replaceChildren(...(chart?[identity(chart)]:[]));pickers[side].close();}if(state.similar)find(false);else render();},first:()=>state.left,useAsFirst:(id,findNow=false)=>{ports.browserState.chooseComparison('right',null);pickers.right.input.value='';pickers.right.selection.replaceChildren();pickers.right.close();choose('left',id);if(findNow){ports.usage?.emit('similar_requested');const u=new URL(location.href);u.searchParams.set('similar','1');ports.historyPort.replaceState(history.state,'',u);find();}},useAsSecond:id=>choose('right',id)};
}
comparison=Object.freeze({mount});
})();
