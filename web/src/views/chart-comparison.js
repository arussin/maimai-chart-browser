...
export function createChartComparison(ports) {
  ...
  (() => {
    ...
    function mount({ data, eligibleIds }) {
      ...
      const bpm = (c) => data.navigation?.charts?.[c.chart_id]?.bpm ?? null,
        bpmText = (c) => (bpm(c) == null ? 'BPM unknown' : bpm(c) + ' BPM');
      let index = null,
        matches = null;
      ...
      el('similar-priority').onchange = () => {
        state.priority = el('similar-priority').value;
        if (matches !== null) {
      ...
      return {
        ...
        useAsSecond: (id) => choose('right', id),
      };
    }
