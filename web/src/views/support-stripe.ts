...
import type { CheckoutState, CheckoutStatus, SupportClient } from './support-client';
export interface SupportLocalization extends Pick<TextView, 'text' | 'attribute'> {
  controls(host: Element): Element;
}
export interface SupportPorts {
  root: HTMLElement;
  localization?: SupportLocalization;
  supportClient?: SupportClient;
  historyPort: Pick<HistoryPort, 'replaceState'>;
}
...
/** Existing checkout behavior, with explicit shell and payment-only ports. */
export function createSupportStripe(ports: SupportPorts): void {
  const root = ports.root,
    document = root.ownerDocument,
    window = document.defaultView!,
    location = window.location;
  const i18n: Pick<TextView, 'text' | 'attribute'> = ports.localization || {
    text: (node, value) => (node.textContent = String(value)),
    attribute: (node, key, value) => node.setAttribute(key, String(value)),
  };

  const client = ports.supportClient,
    candidateOpener = root.querySelector<HTMLElement>('#support-open');
  if (
    !client?.config.enabled ||
    !candidateOpener ||
    candidateOpener.dataset.checkoutReady ||
    typeof HTMLDialogElement === 'undefined' ||
    !HTMLDialogElement.prototype.showModal
  )
    return;
  const opener = candidateOpener;
  ...
  const element = <K extends keyof HTMLElementTagNameMap>(
    ...
    if (text) i18n.text(node, text);
    return node;
  };
  const event = (name: 'support_opened' | 'support_checkout_started' | 'support_success_return') =>
    window.dispatchEvent(new CustomEvent('maimai:support', { detail: name }));
  ...
  const title = element('h2', 'support-dialog-title', config.title);
  title.id = 'support-dialog-title';
  const brand = root.querySelector('.party-brand > span');
  if (brand) {
    title.replaceChildren(brand.cloneNode(true));
    title.classList.add('party-brand');
    i18n.attribute(title, 'aria-label', config.title);
  }
  const close = element('button', 'support-dialog-close', '×');
  close.type = 'button';
  i18n.attribute(close, 'aria-label', 'Close support checkout');
  header.append(title, close);
  const body = element('div', 'support-stripe-body');
  const message = element('p', 'support-stripe-status');
  message.setAttribute('role', 'status');
  const invitation = element('p', 'support-invitation', config.invitation);
  const mount = element('div', 'support-stripe-mount');
  mount.id = 'support-stripe-mount';
  const retry = element('button', 'support-primary', 'Try again');
  retry.type = 'button';
  retry.hidden = true;
  const again = element('button', 'support-secondary', 'Start a new checkout');
  again.type = 'button';
  again.hidden = true;
  ...
  opener.dataset.checkoutReady = 'stripe';
  opener.setAttribute('aria-haspopup', 'dialog');
  const provider = opener.parentElement!.querySelector<HTMLElement>('.support-button-provider');
  ...
  let instance: EmbeddedCheckout | null = null;
  let generation = 0,
    busy = false;
  ...
  function result(status: CheckoutStatus, value: CheckoutState) {
    if (status === 'open') return false;
    dialog.dataset.stage = 'result';
    destroy();
    invitation.hidden = true;
    retry.hidden = true;
    again.hidden = status === 'pending';
    if (status === 'paid') {
      i18n.text(message, config.thanks);
      i18n.text(again, 'Support again');
      if (!value.successTracked) {
        value.successTracked = true;
        save(value);
        event('support_success_return');
      }
    } else if (status === 'pending') {
      i18n.text(
        message,
        'Your payment is still processing. Please check its status before trying another payment.',
      );
      i18n.text(retry, 'Check payment status');
      retry.hidden = false;
    } else {
  ...
  function errorMessage(error: unknown): string {
    ...
    return info.name === 'QuotaExceededError' || info.name === 'SecurityError'
      ? 'Checkout needs temporary storage in this tab to return safely from a wallet. Please allow it and try again.'
      : info.name === 'TimeoutError' || info.name === 'AbortError'
        ? 'Checkout is taking too long. Try again to resume the same payment.'
        : error instanceof TypeError
          ? 'Checkout could not connect. Try again to resume the same payment.'
          : typeof info.message === 'string' && safeMessages.includes(info.message)
            ? String(info.message)
            : 'Checkout could not start. Try again to resume the same payment.';
  }
  async function start() {
    ...
    again.hidden = true;
    destroy();
    i18n.text(message, value.session ? 'Checking your checkout…' : 'Opening secure checkout…');
    try {
      save(value);
      if (value.session) {
      ...
      await loadStripe();
      if (token !== generation) return;
      const data = await request('checkout', value);
      if (token !== generation) return;
      if (!/^cs_(test|live)_[a-zA-Z0-9]{10,200}$/.test(data.session ?? ''))
      ...
      const initializing = window.Stripe!(config.publishableKey)
        ...
        checkout = await Promise.race([
          ...
          new Promise<never>((_, reject) => {
            initializationTimer = setTimeout(() => {
              abandoned = true;
              reject(
                new Error('Checkout is taking too long. Try again to resume the same payment.'),
              );
            }, 20000);
          }),
        ]);
      ...
      i18n.text(message, '');
      if (!value.startedTracked) {
        value.startedTracked = true;
        save(value);
        event('support_checkout_started');
      }
    } catch (error) {
      if (token !== generation) return;
      destroy();
      dialog.dataset.stage = 'result';
      i18n.text(message, errorMessage(error));
      i18n.text(retry, 'Try again');
      retry.hidden = false;
    } finally {
  ...
  if (config.returnHash && location.hash === config.returnHash) {
    ...
    if (returning?.session) {
      returning.resume = true;
      save(returning);
    }
  }
  if (returning?.resume) {
    delete returning.resume;
    save(returning);
    open();
  }
}
