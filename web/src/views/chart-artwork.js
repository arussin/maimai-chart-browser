...
const data=ports.publicData??=JSON.parse(document.getElementById('challenge-data').textContent),art=data.artwork;
...
function image(path,className,label){
  ...
  box.replaceChildren(img);img.src=ports.songPages?.asset(path)??path;return box;
}
function jacket(chart){
  ...
  return image(item?.songId===chart.song_id?path:null,'song-jacket','Jacket for '+(ports.catalogQuery?.titleLabel(chart,ports.localization?.locale)||chart.title));
}
