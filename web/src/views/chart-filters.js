...
export function createChartFilters(ports) {
let filterDisclosure,catalogFilters;
...
const disclosureViews=new Map();
filterDisclosure=(root,toggle,body,key)=>{
  ...
  try{disclosure.expanded=(localStorage.getItem(key)??sessionStorage.getItem(key))==='0';}catch{}
  function update(persist=false){
    if(persist)try{localStorage.setItem(key,disclosure.expanded?'0':'1');}catch{}
    toggle.setAttribute('aria-expanded',String(disclosure.expanded));root.classList.toggle('is-collapsed',!disclosure.expanded);body.inert=!disclosure.expanded;
    i18n.text(hint,disclosure.expanded?'Collapse':'Expand');i18n.attribute(toggle,'title',disclosure.expanded?'Click to collapse':'Click to expand');
    i18n.attribute(root,'title',disclosure.expanded?'':'Click to expand');
  }
  toggle.onclick=()=>{disclosure.expanded=!disclosure.expanded;update(true);if(disclosure.expanded)ports.usage?.emit('filters_opened',undefined,key.includes('personal')||key.includes('player')?'personal':'catalog');};
  disclosureViews.set(toggle.id,update);update();
};
filterDisclosure.sync=()=>{for(const update of disclosureViews.values())update(true);};
...
function mount(charts,onChange){
  ...
  const selected=ports.browserState.difficulties,difficulties=[...new Set(charts.map(c=>c.difficulty))].sort((a,b)=>order.indexOf(a.toUpperCase())-order.indexOf(b.toUpperCase()));
  ...
  ports.browserState.configureLevels(difficulties,levels);const range=ports.browserState.level;let drag=null;
  ...
  for(const difficulty of difficulties){
    ...
    input.onchange=()=>{if(input.checked)selected.add(difficulty);else selected.delete(difficulty);updateDifficulties();ports.usage?.emit('filter_first_used',undefined,'difficulty');onChange();};
  ...
  function sync(){
    ...
    fields.forEach((field,i)=>{field.value=levels.length?label(levels[i?range.high:range.low]):'';field.removeAttribute('aria-invalid');});
    sliders.forEach((slider,i)=>{slider.value=String(i?range.high:range.low);i18n.attribute(slider, 'aria-valuetext', levels.length?'Level '+label(levels[i?range.high:range.low]):'No levels available');});
    sliders[0].setAttribute('aria-valuemax',String(range.high));sliders[1].setAttribute('aria-valuemin',String(range.low));
    ...
    el('level-clear').disabled=!levels.length||range.low===0&&range.high===levels.length-1;
  }
  function clearLevels(){range.low=0;range.high=Math.max(0,levels.length-1);sync();}
  function setSlider(side,index){
    index=Math.max(0,Math.min(levels.length-1,index));
    if(side===0)range.low=Math.min(index,range.high);else range.high=Math.max(index,range.low);
    sync();ports.usage?.emit('filter_first_used',undefined,'level');onChange();
  }
  function commit(side){
    ...
    if(index===(side?range.high:range.low)){sync();return;}
    if(side===0){range.low=index;if(range.low>range.high)range.high=range.low;}else{range.high=index;if(range.high<range.low)range.low=range.high;}
    sync();ports.usage?.emit('filter_first_used',undefined,'level');onChange();
  }
  ...
  track.onpointerdown=event=>{
    ...
    const index=trackIndex(event),side=Math.abs(index-range.low)<Math.abs(index-range.high)?0:Math.abs(index-range.low)>Math.abs(index-range.high)?1:index<=range.low?0:1;
  ...
  return {
    ...
    restore(value){ports.browserState.restoreFilters(value);updateDifficulties();sync();},
    sync(){updateDifficulties();sync();},
    matches(chart){
      ...
      const value=number(chart.level);return value!=null&&value>=levels[range.low]&&value<=levels[range.high];
    },
    clear(){selected.clear();updateDifficulties();clearLevels();},
    activeCount(){return Number(selected.size>0)+Number(range.low>0)+Number(range.high<levels.length-1);},
    chips(){
      const result=[];
      ...
      if(range.low>0)chip('From level '+label(levels[range.low]),'Remove minimum level filter',()=>{range.low=0;sync();onChange();fields[0].focus();});
      if(range.high<levels.length-1)chip('To level '+label(levels[range.high]),'Remove maximum level filter',()=>{range.high=levels.length-1;sync();onChange();fields[1].focus();});
      return result;
    }
  };
}
catalogFilters=Object.freeze({mount,levelNumber:number});
})();
