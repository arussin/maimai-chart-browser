...
export function createChartLinks(ports) {
  ...
  (() => {
    ...
    function configure(index) {
      identityVersion = index?.version === 'mai-notes-links-2' ? 2 : 1;
      playerLinks = ['mai-notes-links-1', 'mai-notes-links-2'].includes(index?.version)
        ? index.charts
        : null;
    }
    ...
    function maiNotes(chart) {
      ...
      if (
        !record ||
        (identityVersion === 1 &&
          (!chart.source_hash || record.source_hash !== chart.source_hash)) ||
