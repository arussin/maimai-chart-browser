...
export function createLocalization(ports) {
  ...
  (() => {
    ...
    const sourceText = (value) =>
      value && typeof value === 'object'
        ? Object.hasOwn(value, 'literal')
          ? value.literal
          : value.message
            ? value.message.replace(/\{(\d+)\}/g, (_, n) => sourceText(value.values[n]))
    ...
    const routeLocale = { en: 'en', ja: 'ja', ko: 'ko', 'zh-hans': 'zh-Hans' }[
      location.pathname.split('/')[1]
    ];
    const linkedLocale = new URLSearchParams(location.search).get('lang');
    if (routeLocale) locale = routeLocale;
    else if (languages.includes(linkedLocale)) locale = linkedLocale;
    ...
    function translate(value) {
      ...
      if (value && typeof value === 'object' && Object.hasOwn(value, 'parts'))
      ...
      if (value && typeof value === 'object' && value.message)
        ...
        ).replace(/\{(\d+)\}/g, (_, n) => translate(value.values[n]));
      ...
      for (const pattern of patterns) {
        ...
        if (match) {
          ...
          return (
            leading +
            catalogs[pattern.source][locale].replace(/\{(\d+)\}/g, (_, n) =>
    ...
    function attribute(node, name, source) {
      if (attributes.has(name))
        bind(node, name, source, (target, value) => target.setAttribute(name, value));
    ...
    function controls(host) {
      ...
      for (const button of controls.querySelectorAll('[data-language]'))
        button.setAttribute('aria-pressed', String(button.dataset.language === locale));
      return controls;
    }
    function mount() {
      for (const link of document.querySelectorAll('a[data-localized-readme]'))
      ...
      const header =
        document.querySelector('[data-version-browser] .site-header') ||
    ...
    window.addEventListener('storage', (event) => {
      if (event.key === key && languages.includes(event.newValue))
