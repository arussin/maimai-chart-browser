...
export function createPlayerRanges(ports) {
let playerRanges;
...
(()=>{'use strict';
  ...
  const text=(node,value)=>ports.localization.text(node,value),format=value=>String(Number(value.toFixed(4)));
  ...
  playerRanges=(host,state,onchange,getRatings)=>{
  ...
  function card(kind,keys,title){
    const fields=keys.map(key=>{const field=document.createElement('input');field.type='number';field.min='0';if(kind==='achievement')field.max='101';field.id='personal-'+key;field.value=state[key];field.oninput=()=>{state[key]=field.value;ports.usage?.emit('filter_first_used',undefined,kind);onchange();};return field;});
    ...
    const reset=document.createElement('button');reset.type='button';text(reset,'Clear');ports.localization.attribute(reset,'aria-label',kind==='achievement'?'Remove Achievement filter':'Remove Chart rating filter');
    ...
    fields.forEach((field,index)=>{
      ...
      ports.localization.attribute(field,'aria-label',kind==='achievement'?(index?'Achievement to %':'Achievement from %'):(index?'Chart rating to':'Chart rating from'));
    ...
    const sliders=fields.map((field,index)=>{
      ...
      ports.localization.attribute(slider,'aria-label',kind==='achievement'?(index?'Achievement to %':'Achievement from %'):(index?'Chart rating to':'Chart rating from'));
    ...
    function sync(){
      ...
      fields.forEach((field,index)=>{
        ...
        const grade=kind==='achievement'?ports.maishift.grade(Math.round(value*10000)):'';
        slider.setAttribute('aria-valuetext',(field.value===''?ports.localization.translate('Any')+' · ':'')+format(value)+(kind==='achievement'?'% · '+grade:' RT'));
