...
export function createPatternFilter(ports) {
let patternFilter;
/* Searchable public pattern filters. Multiple selections match any selected pattern. */
(()=>{'use strict';
const i18n=ports.localization||{text:(node,value)=>node.textContent=value,attribute:(node,key,value)=>node.setAttribute(key,value),option:(...args)=>new Option(...args),searchTerms:value=>value,literal:(node,value)=>node.textContent=value};
...
function mount(overview,onChange){
  ...
  const ids=[...overview.patternIds],selected=ports.browserState.patterns,rows=[];
  ...
  for(const [index,id] of ids.entries()){
    ...
    input.onchange=()=>{ports.usage?.emit('filter_first_used',undefined,'pattern');if(input.checked)selected.add(id);else selected.delete(id);update();onChange();};
  ...
  search.oninput=()=>{if(search.value.trim())ports.usage?.emit('search_used',undefined,'charts');update();};
  ...
  function set(values){ports.browserState.setPatterns(values);search.value='';update();}
...
patternFilter=Object.freeze({mount});
})();
