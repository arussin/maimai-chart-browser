  ...
  (() => {
    ...
    function mount(overview, onChange) {
      const definitions = new Map(
        JSON.parse(el('pattern-data').textContent).map((p) => [p.pattern_id, p]),
      );
      ...
      const keepVisible = () =>
        requestAnimationFrame(() => {
          if (menu.open)
            menu
              .querySelector('.pattern-filter-panel')
      ...
      menu.addEventListener('keydown', (event) => {
        ...
        const next =
          event.target === search
            ...
            : Math.max(
                0,
                Math.min(inputs.length - 1, index + (event.key === 'ArrowDown' ? 1 : -1)),
              );
        if (inputs[next]) {
          event.preventDefault();
          inputs[next].focus();
      ...
      menu.addEventListener('toggle', () => {
        if (menu.open) {
          ...
          const rect = summary.getBoundingClientRect(),
            ...
            height = menu.querySelector('.pattern-filter-panel').getBoundingClientRect().height;
