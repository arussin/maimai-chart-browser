...
export function createRegistryBrowser(ports) {
  ...
  (() => {
    ...
    function matchesRegion(chart, region) {
    ...
    function mount(data, changed) {
      ...
      const controls = document.getElementById('regional-controls');
      ...
      function apply(notify = true) {
        for (const chart of data.catalog) {
          ...
          Object.assign(data.navigation.charts[chart.chart_id], projected.navigation);
        }
        if (notify) changed();
      }
      checkbox.onchange = () => {
