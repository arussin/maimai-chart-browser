...
/** Shared authored illustrations. These examples never constitute chart evidence. */
export function createChartVisuals(ports: { configuration: { theme: ChartTheme } }) {
  ...
  const tap = (x: number, y: number) =>
    `<circle class="pattern-example-tap" cx="${x}" cy="${y}" r="7"/>`;
  const star = (x: number, y: number) =>
    `<polygon class="pattern-example-tap" points="${[
      ...
      .map(([dx, dy]) => `${x + dx},${y + dy}`)
      .join(' ')}"/>`;
  const line = (x1: number, y1: number, x2: number, y2: number, cls = 'pattern-example-line') =>
  ...
  function patternSvg(id: string) {
    ...
    switch (id) {
      ...
      case 'pattern.same_position_repetition':
        drawing = lanes() + [40, 86, 132, 178, 224, 270].map((x) => tap(x, 27)).join('') + timing();
        break;
      ...
      case 'trait.slide_occupancy':
        ...
        break;
      default: {
        const heightExamples: Record<string, number[]> = {
        ...
        const heights = heightExamples[id];
        drawing =
          heights
            .map(
              (h, i) =>
  ...
  return Object.freeze({
    ...
    difficultyColors,
    patternSvg,
    patternSummary: (id: string) => (Object.hasOwn(summaries, id) ? summaries[id] : ''),
  });
}
