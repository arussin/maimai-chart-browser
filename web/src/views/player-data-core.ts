import type {
  ...
  PlayerRecord,
  PlayerSnapshot,
} from '../domain/player-data';
import type { PlayerOffer } from '../runtime/player-session-contracts';
/** Portable data validation and deterministic merging. No DOM, persistence or provider requests. */
export function createPlayerDataCore(_ports: Record<string, never> = {}) {
  ...
  const text = (v: unknown, max = 512, empty = false) =>
    typeof v === 'string' &&
    v.length <= max &&
    (empty || v.length > 0) &&
    !/[\x00-\x1f\uD800-\uDFFF]/u.test(v);
  const integer = (v: unknown, max = Number.MAX_SAFE_INTEGER, nullable = false) =>
    (nullable && v === null) ||
  ...
  async function hash(v: string | Uint8Array | ArrayBuffer) {
  ...
  const recordFields: (keyof PlayerRecord)[] = [
    'chartID',
    'achievement',
    'grade',
    'rate',
  ...
  const chartFields: (keyof PlayerChart)[] = [
    'chartID',
    'songID',
    'title',
    'artist',
  ...
  function validPlayer(input: unknown): input is PlayerIdentity {
    const p = input as PlayerIdentity;
    if (
    ...
    if (p.provider === 'maishift')
      return ['jp', 'intl'].some(
        (region) => p.key === 'maishift:maimaidx:' + region + ':' + encodeURIComponent(p.username),
      );
    return false;
  }
  function structure(input: unknown): PlayerDataset {
    const d = input as PlayerDataset;
    if (
      !keys(d, [
        'format',
        'schemaVersion',
    ...
    for (const name of ['charts', 'records', 'plays', 'snapshots', 'captures'] as const)
      if (
        !object(d[name]) ||
    ...
    for (const [id, c] of Object.entries(d.charts))
      if (
        ...
        !['chartID', 'songID', 'format', 'difficulty'].every((k) =>
          text(c[k as keyof typeof c], 256),
        ) ||
        !['title', 'artist', 'level', 'displayVersion'].every((k) =>
          text(c[k as keyof typeof c], 512, true),
        ) ||
    ...
    for (const [id, r] of Object.entries(d.records)) {
      ...
      for (const k of ['grade', 'lamp', 'sync', 'displayVersion'])
        if (!text(r[k as keyof typeof r], 512, true)) fail('Invalid score text.');
  ...
  async function decode(input: Uint8Array | ArrayBuffer) {
    ...
    try {
      ...
      throw new Error((e instanceof Error ? e.message : '') || 'Invalid compressed player file.');
    }
  }
  async function encode(input: unknown) {
    const d = structure(input);
  ...
  function current(d: PlayerDataset) {
  ...
  function chartHistory(d: PlayerDataset, chartIDs: readonly string[]) {
    ...
    const changes: { id: string; time: number; r: PlayerRecord }[] = [];
    let previous: string | null = null;
    let previousRecord: PlayerRecord | null = null;
    for (const [id, s] of Object.entries(d.snapshots).sort(
      ([a, x], [b, y]) =>
        x.capturedAt - y.capturedAt ||
        Number(x.phase === 'after') - Number(y.phase === 'after') ||
  ...
  function profile(
    d: PlayerDataset,
    pbs: ReadonlyMap<string, PlayerRecord>,
  ...
  function offer(d: PlayerDataset) {
  ...
  function validateOffer(input: unknown): PlayerOffer {
    const o = input as PlayerOffer;
    if (
      !object(o) ||
    ...
    for (const k of ['snapshotIDs', 'captureIDs'] as const)
      if (
        !Array.isArray(o[k]) ||
        o[k].length > 100000 ||
  ...
  function needsUpdate(d: PlayerDataset | null, o: PlayerOffer) {
    validateOffer(o);
  ...
  function observationDates(d: PlayerDataset) {
    const dates: Record<'charts' | 'plays', Record<string, number>> = { charts: {}, plays: {} };
  ...
  async function reconcile(d: PlayerDataset) {
            ...
            candidates = sources.filter((source) =>
              Object.entries(weak).every(
                ([field, value]) =>
                  value === null ||
                  value === '' ||
                  value === d.records[d.plays[source]][field as keyof PlayerRecord],
              ),
            );
  ...
  async function merge(a: PlayerDataset, b: PlayerDataset) {
    structure(a);
    structure(b);
    ...
    for (const name of ['records', 'snapshots', 'captures'] as const)
      for (const [id, row] of Object.entries(b[name])) {
        ...
        (d[name] as Record<string, typeof row>)[id] = row;
      }
    for (const name of ['charts', 'plays'] as const)
      for (const [id, row] of Object.entries(b[name])) {
        const old = d[name][id];
        if (
          old === undefined ||
