...
export function createSettingsMenu(ports) {
let settings;
...
(() => {
  ...
  document.getElementById('report-issue')?.addEventListener('click',()=>ports.usage?.emit('report_issue_opened'));
...
(() => {
  'use strict';
  const trigger = document.getElementById('site-share');
  if (!trigger || document.getElementById('site-share-dialog')) return;
  ...
  const i18n = ports.localization || {
    ...
    attribute: (node, name, value) => node.setAttribute(name, value),
  ...
  function icon(path, filled = false) {
    ...
    if (!filled) {
      shape.setAttribute('stroke', 'currentColor'); shape.setAttribute('stroke-width', '1.8');
  ...
  const title = make('h2', 'Share this site'); title.id = 'site-share-title';
  const close = make('button', undefined, 'site-share-close'); close.type = 'button';
  i18n.attribute(close, 'aria-label', 'Close');
  close.append(icon(`M6 6l12 12M6 18 18 6`));
  close.id = 'site-share-close'; close.onclick = () => dialog.close();
  header.append(title, close);
  const description = make('p', 'Share the public homepage. Your player data and current search are not included.');
  ...
  const link = make('input'); link.type = 'url'; link.value = siteURL; link.readOnly = true;
  link.id = 'site-share-url'; i18n.attribute(link, 'aria-label', 'Website link');
  link.onclick = () => link.select();
  const copy = make('button'); copy.type = 'button'; copy.id = 'site-share-copy';
  copy.append(icon(`M9 9h11v11H9zM15 9V4H4v11h5`), make('span', 'Copy link'));
  linkRow.append(link, copy);
  const status = make('p', '', 'site-share-status'); status.id = 'site-share-status';
  status.setAttribute('role', 'status'); status.setAttribute('aria-atomic', 'true');
  const actions = make('div', undefined, 'site-share-actions');
  const native = make('button'); native.type = 'button'; native.id = 'site-share-native';
  native.append(icon(`M12 15V3m-4 4 4-4 4 4M7 11H4v10h16V11h-3`), make('span', 'Share with device…'));
  const more = make('a'); more.id = 'site-share-more';
  more.append(icon(`M5 12h.01M12 12h.01M19 12h.01`), make('span', 'More…'));
  actions.append(native, more);
  const note = make('p', 'Social services open in a new tab. More options are provided by AddToAny.', 'site-share-note');
  dialog.append(header, services, linkRow, status, actions, description, note);
  document.body.append(dialog);
  let pending = false;
  const names = {line: 'LINE', x: 'X', hatena: 'Hatena', kakao: 'KakaoTalk', naver: 'Naver',
    wechat: 'WeChat', sina_weibo: 'Weibo', qzone: 'Qzone', facebook: 'Facebook',
    whatsapp: 'WhatsApp', facebook_messenger: 'Messenger', reddit: 'Reddit'};
  ...
  const suggestions = {ja: ['line', 'x', 'hatena'], ko: ['kakao', 'naver'],
    zh: ['wechat', 'sina_weibo', 'qzone'], en: ['facebook', 'whatsapp', 'reddit', 'x', 'facebook_messenger']};
  function locale() {
  ...
  function destination(service) {
    ...
    const url = new URL(naver ? 'https://share.naver.com/web/shareView' :
      service === 'more' ? 'https://www.addtoany.com/share' : 'https://www.addtoany.com/add_to/' + service);
    url.searchParams.set(naver ? 'url' : 'linkurl', siteURL);
    url.searchParams.set(naver ? 'title' : 'linkname', siteTitle);
    return url.href;
  }
  function external(anchor, service) {
  ...
  function render() {
    ...
    for (const service of suggestions[locale()]) {
      ...
      external(anchor, service); services.append(anchor);
    }
    native.hidden = typeof navigator.share !== 'function';
  }
  function fallback() {
    render(); i18n.text(status, '');
    if (!dialog.open) dialog.showModal();
    copy.focus();
  }
  function data() {
    return {url: siteURL, title: siteTitle,
  ...
  async function shareNative() {
    ...
    try {
      ...
      if (dialog.open) dialog.close(); else settings?.focus();
    } catch (error) {
      ...
      } else {ports.usage?.emit('share_native_result',undefined,'failed');fallback();}
    } finally {
  ...
  native.onclick = () => { void shareNative(); };
  copy.onclick = async () => {
    copy.disabled = true;
    try {
  ...
  dialog.addEventListener('keydown', event => {
    ...
    const controls = [...dialog.querySelectorAll('button,a[href],input')]
      .filter(node => !node.hidden && !node.disabled && node.getClientRects().length);
  ...
  window.addEventListener('maimai-language-change', () => { if (dialog.open) render(); });
})();
