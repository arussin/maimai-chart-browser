...
export function createSettingsMenu(ports) {
  ...
  (() => {
    ...
    const items = () =>
      [...menu.querySelectorAll('[role="menuitem"]')].filter((item) => !item.hidden);
    ...
    document
      ...
      ?.addEventListener('click', () => ports.usage?.emit('report_issue_opened'));
    ...
    menu.onkeydown = (event) => {
      const entries = items(),
        index = entries.indexOf(document.activeElement);
  ...
  (() => {
    ...
    dialog.addEventListener('keydown', (event) => {
      ...
      const controls = [...dialog.querySelectorAll('button,a[href],input')].filter(
        (node) => !node.hidden && !node.disabled && node.getClientRects().length,
      );
