...
export interface PlayerStorage {
  ...
  claim(
    ...
    id: string;
    active: StoredPlayer & { source: PlayerSource };
    token: StoreToken;
  } | null>;
...
const conflict = () =>
  new Error('Player data changed in another tab. Reload before importing again.');
...
function matches(
  expected: StoreToken,
  active: StoredPlayer | null,
  control: StoreControl,
): boolean {
  return (
    expected.revision === (active?.revision ?? null) &&
    expected.epoch === control.epoch &&
    expected.version === control.version
  );
}
...
export function createPlayerStorage(ports: StoragePorts = {}): PlayerStorage {
  ...
  function open(): Promise<IDBDatabase> {
    return new Promise((resolve, reject) => {
      ...
      request.onblocked = () =>
        reject(new Error('Close other maimai.party tabs to update device storage.'));
    });
  }
    ...
    try {
      return await new Promise<T>((resolve, reject) => {
        ...
        const ready = () => {
          if (++count !== 2) return;
          try {
            result = run(active.result ?? null, control.result ?? defaults(), store);
          } catch (cause) {
            error = cause;
            transaction.abort();
          }
        ...
        transaction.onerror = () => {};
        transaction.onabort = () =>
          reject(error || new Error('Device storage could not be updated. Nothing was replaced.'));
      });
    } finally {
      database.close();
    }
  }
  const read: PlayerStorage['read'] = () =>
    transaction('readonly', (active, control) => ({
      active,
      control,
      token: token(active, control),
    }));
  const begin: PlayerStorage['begin'] = (expected) =>
    transaction('readwrite', (active, control, store) => {
  ...
  const verify: PlayerStorage['verify'] = (expected) =>
    transaction('readonly', (active, control) => {
      if (!matches(expected, active, control)) throw conflict();
      return token(active, control);
    });
  const save: PlayerStorage['save'] = (value, expected, id = null) =>
    transaction('readwrite', (active, control, store) => {
  ...
  const forget: PlayerStorage['forget'] = (expected = null, clear = false) =>
    transaction('readwrite', (active, control, store) => {
  ...
  const claim: PlayerStorage['claim'] = (expected, manual, now = clock()) =>
    transaction('readwrite', (active, control, store) => {
      if (!matches(expected, active, control)) throw conflict();
      const source = active?.source;
      if (!active || !source?.autoRefresh) return null;
      if (
        (control.lease && control.lease.until > now) ||
        (source.retryAt ?? 0) > now ||
        now - (source.lastAttempt ?? 0) < (manual ? 30000 : 900000)
      )
        return null;
      const id = leaseID();
  ...
  const finish: PlayerStorage['finish'] = (expected, id, patch) =>
    transaction('readwrite', (active, control, store) => {
      if (!matches(expected, active, control) || control.lease?.id !== id || !active?.source)
        throw conflict();
