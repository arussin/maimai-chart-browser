...
export function createFeatureAnnouncements(ports) {
...
const i18n=ports.localization,settings=document.getElementById('settings-toggle'),menu=document.getElementById('settings-actions');if(!settings||!menu)return;
const registry=[{id:'player-import-sources-v1',enabled:()=>Object.values(ports.playerSources.capabilities).every(Boolean),title:'Bring your scores to maimai.party',body:'Bring your Session Report or public Maishift scores into the chart browser. Choose Import player data to get started.'}];
...
replay.onclick=()=>{ports.settings?.close();const item=registry.find(item=>item.enabled());if(item)show(item,true);};
