...
export function createFeatureAnnouncements(ports) {
  ...
  (() => {
    ...
    const i18n = ports.localization,
      ...
      menu = document.getElementById('settings-actions');
    ...
    function position() {
      ...
      bubble.style.maxHeight = Math.max(80, innerHeight - r.bottom - 22) + 'px';
    }
    function show(item, replay = false) {
      if (
        !readyToShow ||
        !item.enabled() ||
      ...
      start.onclick = () => {
        close();
        document.getElementById('player-import')?.click();
      };
      ...
      requestAnimationFrame(() =>
        requestAnimationFrame(() => {
          if (current === item && !bubble.hidden && !document.querySelector('dialog[open]'))
            mark(item.id);
        }),
      );
    ...
    menu.insertBefore(replay, document.getElementById('site-share'));
    replay.onclick = () => {
