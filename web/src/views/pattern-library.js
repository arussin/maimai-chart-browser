...
(()=>{'use strict';
const i18n=ports.localization||{text:(node,value)=>node.textContent=value,attribute:(node,key,value)=>node.setAttribute(key,value),option:(...args)=>new Option(...args),searchTerms:value=>value,parts:(values,separator)=>values.join(separator),message:(source,values)=>source.replace(/\{(\d+)\}/g,(_,n)=>values[n]),original:node=>node.textContent,literal:(node,value)=>node.textContent=value};

const definitions=JSON.parse(document.getElementById('pattern-data').textContent),book=ports.configuration.lessons,lessons=book.lessons;
...
function render(){
  ...
  for(const p of shown){const lesson=lessons[p.pattern_id],card=make('article',undefined,'pattern-card');card.dataset.patternId=p.pattern_id;
    ...
    const overview=ports.overview,count=overview?.frequency.get(p.pattern_id)||0,covered=!!overview?.coverage.get(p.pattern_id);
...
function demo(lesson){
  ...
  function setup(){
    ...
    cabinet=model.kind==='notes'?ports.previewField(cabinetModel(model),'Input positions',1):null;
...
function show(id,button=null,navigate=true){
  ...
  if(ports.overview?.coverage.get(id)){const find=make('button','Find charts with this pattern');find.onclick=()=>{dialog.close();onDiscover(id);};dialog.append(find);}
  close.focus();if(navigate)onNavigate(id);return true;
}
dialog.addEventListener('close',()=>{dispose();if(opener?.isConnected)opener.focus();onNavigate(null);});el('pattern-search').oninput=()=>{if(el('pattern-search').value.trim())ports.usage?.emit('search_used','patterns','patterns');render();};el('pattern-scope').onchange=()=>{ports.usage?.emit('filter_first_used','patterns','pattern');render();};
patternLibrary={render,show,stop:()=>dispose(),setNavigation:callback=>{onNavigate=callback;},setDiscovery:callback=>{onDiscover=callback;},has:id=>definitions.some(p=>p.pattern_id===id)};
})();
