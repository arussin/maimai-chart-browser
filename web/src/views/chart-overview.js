...
(()=>{'use strict';
const i18n=ports.localization||{text:(node,value)=>node.textContent=value,attribute:(node,key,value)=>node.setAttribute(key,value),option:(...args)=>new Option(...args),verbatim:value=>value,parts:(values,separator)=>values.join(separator),message:(source,values)=>source.replace(/\{(\d+)\}/g,(_,n)=>values[n]),literal:(node,value)=>node.textContent=value};

const data=ports.publicData??=JSON.parse(document.getElementById('challenge-data').textContent),pack=data.analysis;
...
const sectionKey=ports.playerContext?.key('maimai-chart-sections-v1')||'maimai-chart-sections-v1',sectionState=ports.browserState.sections;let sectionSerial=0;
...
function section(kind,title,decoration){
  ...
  button.onclick=()=>{sectionState[kind]=!sectionState[kind];if(sectionState[kind])ports.usage?.emit('chart_section_opened',undefined,kind);try{localStorage.setItem(sectionKey,JSON.stringify(sectionState));}catch{}syncSections();};
  return {root,content};
}
const svg=(tag,attrs,text)=>{const n=document.createElementNS('http://www.w3.org/2000/svg',tag);for(const[k,v]of Object.entries(attrs))i18n.attribute(n,k,v);if(text!==undefined)i18n.text(n, text);return n;};
...
const delivery=ports.catalogDetails,pendingGraphs=new Map();let graphFrame=0;
...
function patternButton(id,text=name(id)){const b=make('button',text,'pattern-chip');b.type='button';b.dataset.pattern=id;b.onclick=()=>ports.patternLibrary.show(id,b);return b;}
...
function graph(c,{compact=false,maximum=null,span=null}={}){
  ...
  const chartName=i18n.parts([i18n.verbatim(ports.catalogQuery?.titleLabel(c,ports.localization?.locale)||c.title.trim()||'untitled chart'),c.difficulty],' '),title=i18n.message('Input activity through {0}',[chartName]);
  const art=svg('svg',{viewBox:'0 0 244 '+height,role:'img',preserveAspectRatio:compact?'none':'xMidYMid meet','aria-label':i18n.message('Input activity through {0}. {1} to {2}. Peak {3} inputs per second.',[chartName,i18n.verbatim(clock(record.span[0])),i18n.verbatim(clock(record.span[1])),i18n.verbatim(peak.toFixed(1))])});
...
function pair(left,right){
  ...
  if(ids.length){const table=make('table',undefined,'pattern-metrics'),head=make('thead'),labels=make('tr');for(const text of ['Pattern frequency',...[left,right].map(c=>i18n.parts([i18n.verbatim(c.title),c.difficulty],' · '))]){const th=make('th',text);th.scope='col';labels.append(th);}head.append(labels);table.append(head);const body=make('tbody');
  ...
  for(const c of [left,right]){const figure=make('div');figure.append(make('h3',i18n.parts([i18n.verbatim(ports.catalogQuery?.titleLabel(c,ports.localization?.locale)||c.title.trim()||'Untitled'),c.difficulty],' · ')),graph(c,{maximum:max}));graphs.append(figure);}box.append(graphs);return box;
}
overview=Object.freeze({get,tags,detected,name,chips,graph,details,section,compare,pair,frequency,coverage,patternIds:pack?.patterns||[],definition:id=>pack?.definitions?.[id]||null});
})();
