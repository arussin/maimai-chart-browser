...
const capabilities=Object.freeze({file:true,report:true,maishift:ports.features?.maishift===true||ports.playerContext?.pilot===true});
...
async function readMaishift(value,{signal=new AbortController().signal,manual=true,expectedPlayer=null}={}){
  ...
  const adapter=ports.maishift,selected=adapter.location(value.url||value.handle,value.region);
...
playerSources=Object.freeze({capabilities,reportURL,validateSource,connection,readReport,readMaishift,readSource,AUTO_INTERVAL,MANUAL_INTERVAL,retryAfter});
})();
