...
export function createChallengeReview(ports) {
  ...
  (() => {
    ...
    let data = (ports.publicData ??= JSON.parse(
    ...
    try {
      ...
      if (!status) {
        ...
        document.getElementById('catalog').prepend(status);
      }
      status.dataset.catalogError = 'genre';
      i18n.text(status, error.message);
      ports.publicData = undefined;
      return;
    }
    ...
    function field(snippet, label, lead = 600000) {
    ...
    const values = {
      title: (c) => displayTitle(c),
      artist: (c) => c.artist,
      constant: chartConstant,
      bpm: (c) => navigation.charts?.[c.chart_id]?.bpm ?? null,
      difficulty: (c) => ports.catalogFilters.levelNumber(c.level),
      format: (c) => c.format,
      genre: (c) => genreLabel(folderValue(c, 'genre')),
      version: (c) => {
        const v = (navigation.versions || []).indexOf(folderValue(c, 'version'));
        return v < 0 ? null : v;
      },
      speed: (c) => c.demand?.cadence?.mean_onsets_s ?? null,
      peak: (c) => {
    ...
    Object.assign(values, {
      achievement: (c) => personal?.record(c)?.achievement ?? null,
      grade: (c) => personal?.gradeIndex(c) ?? null,
      rating: (c) => personal?.record(c)?.rate ?? null,
      lastPlayed: (c) => personal?.lastPlayed(c) ?? null,
    });
    ...
    function compareCharts(a, b, rules) {
      for (const rule of rules) {
        ...
        const diff = typeof av === 'number' ? av - bv : collator.compare(av, bv);
    ...
    function renderSort() {
      ...
      state.sortRules.forEach((rule, index) => {
        ...
        b.onclick = () => {
          ...
          renderSort();
          catalog();
          document.querySelector('[data-sort-key="' + rule.key + '"]').focus();
        };
        root.append(b);
      });
    ...
    function initializeFilters() {
      ports.filterDisclosure(
        ...
        el('catalog-filter-content').firstElementChild,
      ...
      document.addEventListener('click', (event) => {
        if (!el('version-filter').contains(event.target)) el('version-filter').open = false;
      });
    ...
    function activeFilters() {
      ...
      if (international?.checked)
        chip('Use maimai international data', () => international.click());
      ...
      for (const id of filters) {
        ...
        const button = make(
          'button',
          i18n.original(select.selectedOptions[0]) + ' ×',
          'filter-chip',
        );
        i18n.attribute(
          button,
      ...
      const existing = new Map(
        [...root.children].map((node) => [node.getAttribute('aria-label'), node]),
      );
    ...
    function catalog(focusKey = null) {
      ...
      for (const [index, { key, chart }] of rows.slice(0, state.visible).entries()) {
        ...
        function renderRow(c) {
          ...
          row.dataset.constant = chartConstant(c) ?? '';
          ...
          const titleLine = make('span', undefined, 'song-title-line'),
            reading = ports.songSearch.romaji(c);
          ...
          identity.append(
            ...
            make(
              ...
              i18n.parts(
                [
                  c.artist ? i18n.verbatim(c.artist) : 'Artist not provided',
                  i18n.verbatim(c.format),
                ],
                ' · ',
              ),
              'muted',
          ...
          const heading = make('div', undefined, 'chart-row-heading'),
            videoLink = ports.chartLinks.group(c);
          heading.append(
            ports.artwork.jacket(c),
            summary,
            overview.chips(c, 3, patternFilter.ids()),
          );
          if (videoLink) heading.append(videoLink);
          const picker = make('select', undefined, 'row-difficulty');
          picker.id = 'row-difficulty-' + index;
          i18n.attribute(
            picker,
            'aria-label',
            'Difficulty for ' + displayTitle(c) + ' ' + c.format,
          );
          ...
          picker.onchange = () => {
            ...
            if (!next) return;
            if (state.expandedRows.has(key)) ports.usage?.emit('chart_opened');
            state.selectedCharts.set(key, next.chart_id);
            const replacement = renderRow(next);
            row.replaceWith(replacement);
            replacement.querySelector('.row-difficulty').focus({ preventScroll: true });
          };
          ...
          i18n.attribute(
            ...
            chartConstant(c) == null
              ...
              : (() => {
                  const source = navigation.charts?.[c.chart_id]?.metric_sources?.chart_constant;
                  return source
          ...
          i18n.attribute(
            speed,
            'aria-label',
            (values.speed(c) == null ? 'Unknown' : values.speed(c).toFixed(1)) +
              ' inputs per second',
          );
          ...
          const versionArt = ports.artwork.version(version);
          ...
          const actions = make('div', undefined, 'chart-detail-actions'),
          ...
          compareButton.onclick = () => {
            selectView('compare');
            if (comparisonUI.first() && comparisonUI.first() !== c.chart_id)
              comparisonUI.useAsSecond(c.chart_id);
            else comparisonUI.useAsFirst(c.chart_id);
          };
          ...
          i18n.attribute(
            ...
            'title',
            c.demand
              ? 'Find charts with comparable measurements'
              : 'Similarity needs prepared analysis',
          );
          similarButton.onclick = () => {
            selectView('compare');
            comparisonUI.useAsFirst(c.chart_id, true);
          };
          actions.append(compareButton, similarButton);
          footer.append(metadata, actions);
          header.append(footer);
          if (personal) header.append(personal.summary(c));
          header.onclick = (event) => {
            if (!event.target.closest('button,select,label,input,a')) summary.click();
          };
          const panel = make('div', undefined, 'chart-measurements');
          panel.id = 'chart-' + index;
          panel.hidden = !state.expandedRows.has(key);
          ...
          const renderDetails = () => {
            ...
            if (ports.songPages) track.content.append(ports.songPages.link(c.song_id, c.chart_id));
          ...
          summary.onclick = () => {
            ...
            row.classList.toggle('is-expanded', !panel.hidden);
            summary.setAttribute('aria-expanded', String(!panel.hidden));
            if (panel.hidden) state.expandedRows.delete(key);
            else {
              state.expandedRows.add(key);
              renderDetails();
              ports.usage?.emit('chart_opened');
            }
          };
    ...
    for (const name of ['compare', 'catalog', 'patterns', 'about'])
      el(name + '-tab').onclick = () => selectView(name);
    previewField = field;
    ports.onPreviewField(field);
    initializeFilters();
    ...
    function personalChanged() {
      for (const key of Object.keys(personalSorts)) delete sortFields[key];
      ...
      eligibleIds: () =>
        data.catalog
          .filter((c) => {
            if (state.format !== 'all' && c.format !== state.format) return false;
            if (
              state.selectedVersions.size &&
              !state.selectedVersions.has(folderValue(c, 'version'))
            )
              return false;
            if (!patternFilter.matches(c)) return false;
            if (personal && !personal.matches(c)) return false;
    ...
    function applyRoute() {
      ...
      if (id && p.get('view') === 'catalog') {
        ...
        if (c) {
          ...
          selectView('catalog');
          catalog(id);
          const findRow = () => [...el('songs').children].find((n) => n.dataset.rowKey === id);
          let row = findRow();
          if (!row) {
    ...
    browserState = state.bind({
      readTransient: () => ({
        auxiliary: {
          ...
          menus: ['version-filter', 'difficulty-filter', 'pattern-filter'].map((id) => [
            id,
            el(id).open,
          ]),
        },
      ...
      writeDisclosures(value) {
        ...
        if (value.auxiliary) {
          ...
          for (const [id, open] of value.auxiliary.menus) el(id).open = open;
        }
