...
function bundledFactory(name) {
  ...
  function visit(node) {
    ...
    if (
      ...
      source
        ...
        .includes('Maishift returned an unsupported response. Your saved data was kept.')
    )
      factories.push(node);
  ...
  return runInNewContext('(' + source.slice(node.start, node.end) + ')', { URL })({
    playerCore: {},
  });
}
...
const facade = { URL };
runInNewContext(readFileSync(new URL('player-maishift.js', assets), 'utf8'), facade);
const subjects = [
  ['maintained module', createPlayerMaishift({ playerCore: {} })],
  ['compatibility entry', facade.maimaiPlayerMaishift],
  ['hosted bundle', bundledFactory(hosted)],
  ['offline bundle', bundledFactory(manifest.entries.offline)],
];
...
for (const locale of locales) {
  ...
  for (const [hint, region] of [
    ['intl', 'intl'],
    ['na', 'intl'],
    ['jp', 'jp'],
  ]) {
    for (const separator of ['@', '%40'])
      prefixes.push({ path: locale + separator + hint + '/', region });
  }
}
const expected = (handle, region) => ({
  handle,
  region,
  url: origin + '/en' + (region === 'auto' ? '' : '@' + region) + '/profile/' + handle + '/home',
});
for (const [name, adapter] of subjects) {
      ...
      for (const handle of ['fictional-player', 'MiXeD_09', '0', 'x'.repeat(64)])
        for (const tab of tabs)
          for (const slash of ['', '/']) {
            ...
            assert.deepEqual(
              plain(adapter.location(adapter.location(url).url)),
              expected(handle, region || 'auto'),
              url,
            );
            if (region) {
              assert.throws(
                () => adapter.location(url, region === 'jp' ? 'intl' : 'jp'),
                /do not match/,
              );
            } else
              for (const selected of ['auto', 'intl', 'jp'])
                assert.deepEqual(
                  plain(adapter.location(url, selected)),
                  expected(handle, selected),
                  url,
                );
            count++;
          }
  ...
  test(name + ' rejects unsafe and unsupported locations without inferring region', () => {
    const paths = [
      '/fr/profile/fictional-player',
      '/profile/fictional-player:1/home',
      '/profile/fictional-player%3A1/home',
      '/en%2540jp/profile/fictional-player',
      '/profile/fictional%2fplayer',
      '/profile/fictional-player/unknown',
      '/profile/fictional-player/home/extra',
      '/en//profile/fictional-player',
      '/@jp/profile/fictional-player',
      '/en@us/profile/fictional-player',
      '/profile/한글',
    ];
    for (const value of [
      ...paths.map((path) => origin + path),
      origin + '/profile/fictional-player?private=fictional',
      origin + '/profile/fictional-player#private',
      'http://maimai.shiftpsh.com/profile/fictional-player',
      'https://user:password@maimai.shiftpsh.com/profile/fictional-player',
      'https://maimai.shiftpsh.com:8443/profile/fictional-player',
      'https://maimai.shiftpsh.com.evil.example/profile/fictional-player',
      null,
      undefined,
      42,
      {},
