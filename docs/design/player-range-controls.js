/* Interactive proposal only. Existing numeric fields remain the filter inputs. */
(()=>{'use strict';
  const stops=[0,50,60,70,75,80,90,94,97,98,99,99.5,100,100.5,101];
  const copy={
    achievement:{en:'Drag by grade or enter an exact %.','zh-Hans':'按评级拖动，或输入精确百分比。',ko:'등급별로 드래그하거나 정확한 %를 입력하세요.',ja:'ランク刻みでドラッグ、または達成率を直接入力。'},
    rating:{en:'Based on your imported chart ratings.','zh-Hans':'范围根据已导入的谱面 Rating 自动设置。',ko:'가져온 채보 레이팅에 맞춘 범위입니다.',ja:'読み込んだ譜面レーティングに合わせた範囲です。'},
    empty:{en:'No known chart ratings in this import.','zh-Hans':'导入的数据中没有已知的谱面 Rating。',ko:'가져온 데이터에 알려진 채보 레이팅이 없습니다.',ja:'読み込んだデータに譜面レーティングがありません。'}
  };
  ...
  let cards=[],pending=false,initialized=false,ratings=null;
  function queue(){if(!pending){pending=true;requestAnimationFrame(()=>{pending=false;cards.forEach(card=>card.sync());});}}
  function updateData(){
    let lo=Infinity,hi=-Infinity;
    for(const chart of window.maimaiResearchCatalog?.catalog||[]){
      const rating=window.maimaiPersonal?.record(chart)?.rate;
  ...
  function card(kind,keys,title){
    const fields=keys.map(key=>document.getElementById('personal-'+key)),host=fields[0].closest('.player-filter-fields');
    ...
    fields.forEach((field,index)=>{
      const old=field.closest('label'),label=document.createElement('label'),inputWrap=document.createElement('span');
      ...
      label.append(inputWrap);values.append(label);old.remove();field.addEventListener('input',queue);
    });
    root.append(values);
    ...
    reset.onclick=()=>{fields.forEach(field=>{field.value='';field.dispatchEvent(new Event('input',{bubbles:true}));});sync();};
    function sync(){
      ...
      hint.textContent=copy[empty?'empty':kind][maimaiI18n.locale||'en'];reset.disabled=fields.every(field=>field.value==='');
    }
    sync();return {sync,root};
  }
  function initialize(){
    if(initialized||!document.getElementById('personal-rateMax'))return;
    initialized=true;cards=[card('achievement',['min','max'],'Achievement'),card('rating',['rateMin','rateMax'],'Chart rating')];
    const sections=document.createElement('div');sections.className='personal-range-sections';sections.append(...cards.map(card=>card.root));document.querySelector('.player-filter-fields').append(sections);
    document.querySelector('.player-filters').addEventListener('click',queue);updateData();
  }
  const observer=new MutationObserver(()=>{initialize();if(initialized)observer.disconnect();});observer.observe(document.body,{childList:true,subtree:true});initialize();
  window.addEventListener('maimai-personal-change',()=>{initialize();updateData();});window.addEventListener('maimai-language-change',queue);
})();
