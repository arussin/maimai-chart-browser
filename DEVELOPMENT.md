...
outputs enumerated by `src/maimai_intelligence/assets/browser-assets.json` and
`web/generated-assets.json`, then promote only those generated outputs. Remove
superseded build-owned chunks only after checking their recorded hashes. `npm run check` rejects stale generated bytes.
...
The shared song/browser ownership and immutable song-data contract are documented
