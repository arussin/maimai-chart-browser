...
def read_registry_document(path):
...
def _read_checkpoint(root: Path | str, identifier: str) -> tuple[Path, dict[str, Any]]:
    ...
    _validate_staged(root, path, manifest)
    return path, manifest


def _validate_staged(root, path, manifest):
...
def restore_checkpoint(
    ...
    identifier = _recover_completion(root, base, policy, identifier)
    if identifier is None:
        return deepcopy(base), None
    path, manifest = _read_checkpoint(root, identifier)
    previous = read_registry_document(path / "result.json")
...
def replay_checkpoint_start(
    ...
    path, manifest = _read_checkpoint(root, receipt["checkpoint"])
...
def commit_checkpoint(
    ...
    *,
    ...
    _validate_staged(root, destination, manifest)
    ...
    _read_checkpoint(root, identifier)
...
def prepare_checkpoint_batch(
    value, published, store, output, *, roots=(), reviews=None, fetcher=None, now=None
):
    ...
    policy = policy_identity(CONFIG, reviews)
    starting, parent = restore_checkpoint(root, value, policy)
    work = checkpoint_work(root, parent) if parent else None
    ...
    write_registry(result, output / "registry")
