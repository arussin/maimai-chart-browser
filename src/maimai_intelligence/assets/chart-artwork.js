...
const bundledVersions={'maimai DX MAGiCAL':'version-magical.png'};
...
function image(path,className,label){
  ...
  if(!(className==='version-logo'&&Object.values(bundledVersions).includes(path))&&(art?.version!=='public-artwork-1'||!/^media\/[a-f0-9]{64}\.webp$/.test(path||'')||!art.assets?.[path]))return box;
...
function version(name){const box=image(art?.versions?.[name]||bundledVersions[name],'version-logo',name+' logo');box.setAttribute('aria-hidden','true');box.removeAttribute('role');box.removeAttribute('aria-label');return box;}
