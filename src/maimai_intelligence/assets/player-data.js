...
function profileCard(offer){
  ...
  if(rating!=null){
    ...
    badge.dataset.tier=tiers.findLast(([floor])=>rating>=floor)[1];badge.setAttribute('role','img');i18n.attribute(badge, 'aria-label', 'Reconstructed rating '+rating);
...
function badges(r){
  ...
  for(const entry of [lamp[r.lamp],sync[syncKey(r.sync)]])if(entry){const icon=make('span',undefined,'player-icon');icon.dataset.icon=entry[0];icon.setAttribute('role','img');i18n.attribute(icon, 'aria-label', entry[1]);i18n.attribute(icon, 'title', entry[1]);fragment.append(icon);}return fragment;
}
function summary(c){const root=make('div',undefined,'player-achievement');root.hidden=!active||!visible;if(root.hidden)return root;root.append(make('span','You','player-personal-label'));const r=record(c);if(!r){root.append(make('span',providerID(c)?'No recorded PB':'Personal chart match unavailable'));return root;}
  ...
  if(r.rate!=null){rating.append(document.createTextNode(' '),make('span','RT','player-score-unit'));i18n.attribute(rating, 'title', 'Chart rating');}
...
function details(c){const group=window.maimaiChartOverview.section('player','Your data'),root=group.root,body=group.content;root.classList.add('player-history');root.hidden=!active||!visible;if(root.hidden)return root;body.append(summary(c));const cid=providerID(c);if(!cid)return root;
  ...
  function tableFor(entries,firstLabel,label){
    ...
    const more=make('button','Show more '+label.toLowerCase());more.type='button';const show=()=>{for(const e of entries.slice(shown,shown+50)){const row=make('tr');row.append(make('td',date(e.time)),make('td',e.r.achievement==null?'Unknown':(e.r.achievement/10000).toFixed(4)+'%'));const grade=make('td'),icons=make('td');grade.append(gradeNode(e.r.grade));icons.append(badges(e.r));row.append(grade,make('td',String(e.r.rate??'—')),icons);tbody.append(row);}shown+=50;more.hidden=shown>=entries.length;};more.onclick=show;show();const scroll=make('div',undefined,'player-history-table');scroll.tabIndex=0;scroll.setAttribute('role','region');i18n.attribute(scroll, 'aria-label', label);scroll.append(table);wrap.append(scroll,more);return wrap;
  }
...
function controls(parent,onchange){const root=make('fieldset',undefined,'player-filters');root.dataset.personalControls='';
  ...
  const scope=make('div',undefined,'player-result-scope');scope.setAttribute('role','group');i18n.attribute(scope, 'aria-label', 'Personal chart scope');
  scope.append(make('span','Show'));
  ...
  for(const [value,label]of [['','All charts'],['yes','My PBs'],['no','No PB yet']]){const b=make('button',label);b.type='button';b.dataset.recorded=value;b.onclick=()=>{state.recorded=value;scopeSelection();change();};scope.append(b);}scopeSelection();root.append(scope);
  ...
  function change(){const n=Object.values(state).filter(v=>v instanceof Set?v.size>0:v!=='').length;i18n.text(count, n?`${n} active`:'');onchange();}
  ...
  for(const [key,title,options]of selects){const label=make('label',title),select=make('select');select.id='personal-'+key;for(const [value,text]of options)select.append(i18n.option(text, value));select.value=state[key];select.onchange=()=>{state[key]=select.value;change();};label.append(select);fields.append(label);}
