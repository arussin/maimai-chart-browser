...
function gradeNode(value){
  ...
  const node=make('span',undefined,'player-grade grade-'+kind);node.setAttribute('role','img');node.setAttribute('aria-label',value||'Grade unknown');
...
function details(c){const group=window.maimaiChartOverview.section('player','Your data'),root=group.root,body=group.content;root.classList.add('player-history');root.hidden=!active||!visible;if(root.hidden)return root;body.append(summary(c));const cid=providerID(c);if(!cid)return root;
  ...
  function tableFor(entries,firstLabel,label){
    ...
    const more=make('button','Show more '+label.toLowerCase());more.type='button';const show=()=>{for(const e of entries.slice(shown,shown+50)){const row=make('tr');row.append(make('td',date(e.time)),make('td',e.r.achievement==null?'Unknown':(e.r.achievement/10000).toFixed(4)+'%'));const grade=make('td'),icons=make('td');grade.append(gradeNode(e.r.grade));icons.append(badges(e.r));row.append(grade,make('td',String(e.r.rate??'—')),icons);tbody.append(row);}shown+=50;more.hidden=shown>=entries.length;};more.onclick=show;show();const scroll=make('div',undefined,'player-history-table');scroll.tabIndex=0;scroll.setAttribute('role','region');scroll.setAttribute('aria-label',label);scroll.append(table);wrap.append(scroll,more);return wrap;
  }
  if(plays.length)body.append(tableFor(plays,'Played on','Recorded plays'));
  if(changes.length){const toggle=make('button',`Show saved PB changes (${changes.length})`,'player-pb-toggle'),saved=make('div',undefined,'player-pb-history');toggle.type='button';toggle.setAttribute('aria-expanded','false');saved.hidden=true;saved.id='saved-pbs-'+c.chart_id;toggle.setAttribute('aria-controls',saved.id);saved.append(make('p','These dates show when a PB was saved, not when you played. Unchanged scores are grouped.','muted'),tableFor(changes,'Saved on','Saved PB changes'));toggle.onclick=()=>{saved.hidden=!saved.hidden;toggle.setAttribute('aria-expanded',String(!saved.hidden));toggle.textContent=`${saved.hidden?'Show':'Hide'} saved PB changes (${changes.length})`;};body.append(toggle,saved);}
  return root;
}
...
function controls(parent,onchange){const root=make('fieldset',undefined,'player-filters');root.dataset.personalControls='';
  ...
  const scope=make('div',undefined,'player-result-scope');scope.setAttribute('role','group');scope.setAttribute('aria-label','Personal chart scope');
  ...
  for(const [key,label]of [['rating','Your RT'],['achievement','Achievement'],['grade','Grade'],['lastPlayed','Last recorded play']]){const button=make('button',label);button.type='button';button.dataset.sortKey=key;if(key==='rating')button.title='Sort by your chart rating (RT)';sorting.append(button);}fields.append(sorting);
