...
function saveTab(bytes,epoch=storeToken?.epoch??null,profileRating=null,lastImportedAt=null){let text='';for(let i=0;i<bytes.length;i+=32768)text+=String.fromCharCode(...bytes.subarray(i,i+32768));try{sessionStorage.setItem(storageKey('maimai-player-session'),JSON.stringify({epoch,bytes:btoa(text),profileRating,lastImportedAt}));}catch{throw new Error('This tab’s temporary storage is full or unavailable. Nothing was replaced. You can choose “Remember on this device” to use device storage.');}}
const importTime=value=>Number.isSafeInteger(value)&&value>0&&value<=8640000000000000?value:null;
...
function changed(redraw=true){const scroll=[scrollX,scrollY],expandedHistory=[...document.querySelectorAll('.player-pb-toggle[aria-expanded="true"]')].map(node=>node.getAttribute('aria-controls'));pbs=active?core.current(active).pbs:new Map();pbDates=new Map();if(active)for(const s of Object.values(active.snapshots))for(const cid of Object.keys(s.pbs))pbDates.set(cid,Math.max(pbDates.get(cid)||0,s.capturedAt));lastTimes=new Map();if(active)for(const ref of Object.values(active.plays)){const r=active.records[ref];if(r.timeAchieved!=null)lastTimes.set(r.chartID,Math.max(lastTimes.get(r.chartID)||0,r.timeAchieved));}hideButton.hidden=!active;forgetButton.hidden=!storedRevision;i18n.text(hideButton, visible?'Hide player data':'Show player data');
  clearButton.hidden=!active&&!storedRevision;
  status.hidden=!active;if(active){const o=core.offer(active);const card=profileCard(o,sourceRating);card.classList.add('player-profile-compact');if(source?.type==='report'){const link=external('Session Report',source.url);link.className='player-profile-history player-profile-link';card.append(link);}status.replaceChildren(card,make('small',window.maimaiI18n.message('Last Updated: {0}',[date(lastImportedAt)]),'player-storage-label'));}else status.replaceChildren();
...
function profileCard(offer,reportedRating=null){
  ...
  const reported=offer.player.provider==='maishift'&&reportedRating!=null,rating=reported?reportedRating:offer.profile?.rating,label=reported?'Maishift rating':rating==null?'Rating unknown':'Reconstructed rating';
  {
    ...
    badge.setAttribute('role','img');i18n.attribute(badge, 'aria-label', rating==null?'Rating unknown':reported?window.maimaiI18n.message('Maishift rating {0}',[String(rating)]):'Reconstructed rating '+rating);
  ...
  if(offer.player.provider==='maishift'){
    ...
    try{username=external(i18n.verbatim(offer.player.username),window.maimaiPlayerMaishift.location(offer.player.username,offer.player.key.split(':')[2]).url);username.className='player-profile-history player-profile-link';}catch{}
    card.append(username,make('span',window.maimaiI18n.message('{0} PBs',[offer.pbCount.toLocaleString()]),'player-profile-history'));return card;
  }
...
function ask(offer,{file=false,stale=false,connection=null,rememberDefault=true,unmatched=null}={}){return new Promise(resolve=>{dialog.replaceChildren();dialog.classList.remove('player-message');dialog.removeAttribute('aria-describedby');const h=make('h2','Import this profile?');h.id='player-dialog-title';dialog.append(h,profileCard(offer,connection?.profileRating),make('p',`Captured ${date(offer.capturedAt)}`,'player-capture-date'));
  ...
  if(active&&active.player.key!==offer.player.key)dialog.append(make('p','Importing will overwrite data from the currently imported account.','player-import-replace-warning'));
  if(connection){
...
async function commit(data,remember,{connection=null,expected=generation,automatic=false,lease=null}={}){
  ...
  if(connection?.type==='maishift'&&active?.player.key===data.player.key){
    if(connection.adapterVersion===2)previous=await window.maimaiPlayerMaishift.enrichRatings(active,data);
    ...
    if(unchanged){data={...previous,player:data.player};const {revision,...body}=data;data.revision=await core.digest(body);}
  ...
  const bytes=await core.encode(next);assertCurrent(expected);const importedAt=Date.now();
  if(remember){if(!storeToken)throw new Error('Device storage is unavailable.');storeToken=await storage.save({revision:next.revision,bytes,source:connection,lastImportedAt:importedAt},storeToken,lease);storedRevision=storeToken.revision;assertCurrent(expected);clearTab();notify('saved');}
  else{if(storeToken)await storage.verify(storeToken);assertCurrent(expected);
    if(remembered&&source&&connection?.url===source.url){
      const previous=sessionStorage.getItem(storageKey('maimai-player-session'));saveTab(bytes,storeToken.epoch+1,connection?.profileRating??null,importedAt);
      try{storeToken=await storage.forget(storeToken);}catch(e){if(previous===null)clearTab();else sessionStorage.setItem(storageKey('maimai-player-session'),previous);throw e;}
      storedRevision=null;notify('forgotten');assertCurrent(expected);
    }else saveTab(bytes,storeToken?.epoch??null,connection?.profileRating??null,importedAt);
  }
...
async function selectSource(){
  ...
  function render(){fields.replaceChildren();next.disabled=selected==='maishift'&&!sources.capabilities.maishift;
    if(selected==='file'){fields.append(make('p','Choose a compressed player file exported by Session Report.'));next.onclick=()=>{dialog.close();input.click();};}
      ...
      next.onclick=()=>{let selected;try{selected=window.maimaiPlayerMaishift.location(url.value);}catch(e){message('Player data could not be imported',e.message);return;}importReport(selected,rememberChoice);};
    }
    else{
  ...
  cancel.onclick=()=>{dialog.close();focusDialogTrigger();};actions.append(next,cancel);dialog.append(heading);if(active)dialog.append(make('p','Importing will overwrite data from the currently imported account.','player-import-replace-warning'));dialog.append(group,fields,actions);render();if(!dialog.open)dialog.showModal();group.querySelector(':checked')?.focus();
}
...
async function refresh(manual=false){
  ...
  refreshing=true;const expected=generation;let lease;try{
    ...
    if(result.data.revision===source.sourceRevision||older||source.type==='maishift'&&result.source.contentRevision===source.contentRevision){
      if(older){source=await storage.finish(storeToken,lease.id,{lastChecked:now,retryAt:null});assertCurrent(expected);}
      else{
        const checked={...source,lastAttempt:lease.active.source.lastAttempt,lastChecked:now,lastSuccess:now,retryAt:null,...(source.type==='maishift'?{adapterVersion:result.source.adapterVersion,sourceUpdatedAt:result.source.sourceUpdatedAt,profileRating:result.source.profileRating,diagnosticCount:result.source.diagnosticCount,diagnostics:result.source.diagnostics}: {})};
        // Commit local freshness without changing portable observations or history.
        storeToken=await storage.save({...lease.active,source:checked,lastImportedAt:now},storeToken,lease.id);assertCurrent(expected);
        source=checked;lastImportedAt=now;notify('saved');
      }
      sourceRating=source?.type==='maishift'?source.profileRating??null:null;
    }else await commit(result.data,true,{connection:updated,expected,automatic:true,lease:lease.id});
    refreshText=older?'The source returned older data. Your saved data was kept.':'';
...
async function receiveUpdate(event){
  if(!['saved','forgotten','cleared','invalidate'].includes(event?.kind))return;
  if(['forgotten','cleared'].includes(event.kind)&&Number.isSafeInteger(event.epoch)&&event.epoch>(storeToken?.epoch??0)){
    invalidate();busy=false;clearTab();remembered=false;source=null;storedRevision=null;refreshText='The saved profile and connection were removed. You can keep viewing these scores in this tab; automatic refresh is off.';if(event.kind==='cleared')clearCurrent();changed(event.kind==='cleared');
  }
  await ready;try{const state=await storage.read();if(state.token.epoch===(storeToken?.epoch??0)&&state.token.version<=(storeToken?.version??0))return;
    const forgotten=state.token.epoch!==(storeToken?.epoch??0),cleared=(state.control.clearedEpoch??0)>(storeToken?.epoch??0),wasRemembered=remembered;invalidate();busy=false;storeToken=state.token;storedRevision=storeToken.revision;
    if(cleared)clearCurrent();
    else if(forgotten){clearTab();remembered=false;source=null;refreshText='The saved profile and connection were removed. You can keep viewing these scores in this tab; automatic refresh is off.';}
    else if(wasRemembered&&state.active&&event.kind==='saved'){
      const expected=generation,data=await core.reconcile(await core.decode(state.active.bytes));assertCurrent(expected);active=data;source=sources.validateSource(state.active.source??null,data.player.key);sourceRating=source?.type==='maishift'?source.profileRating??null:null;lastImportedAt=importTime(state.active.lastImportedAt)??importTime(source?.lastSuccess);
    }
    changed(cleared||event.kind!=='invalidate');
...
window.addEventListener('storage',event=>{if(event.key===storageKey('maimai-player-event'))try{receiveUpdate(JSON.parse(event.newValue));}catch{}});
async function resume(){if(document.visibilityState==='hidden')return;await receiveUpdate({kind:'invalidate'});await refresh();}
...
function details(c){const group=window.maimaiChartOverview.section('player','Your data'),root=group.root,body=group.content;root.classList.add('player-history');root.hidden=!active||!visible;if(root.hidden)return root;body.append(summary(c));const cid=providerID(c);if(!cid)return root;
  ...
  if(changes.length){const toggle=make('button',`Show saved PB changes (${changes.length})`,'player-pb-toggle'),saved=make('div',undefined,'player-pb-history');toggle.type='button';toggle.setAttribute('aria-expanded','false');saved.hidden=true;saved.id='saved-pbs-'+c.chart_id;toggle.setAttribute('aria-controls',saved.id);saved.append(make('p','These dates show when a PB was saved, not when you played. Unchanged scores are grouped.','muted'),tableFor(changes,'Saved on','Saved PB changes'));toggle.onclick=()=>{saved.hidden=!saved.hidden;toggle.setAttribute('aria-expanded',String(!saved.hidden));i18n.text(toggle, `${saved.hidden?'Show':'Hide'} saved PB changes (${changes.length})`);};body.append(toggle,saved);}
  return root;
}
