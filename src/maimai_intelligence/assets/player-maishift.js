...
const gradeThresholds=[[1005000,'SSS+'],[1000000,'SSS'],[995000,'SS+'],[990000,'SS'],[980000,'S+'],[970000,'S'],[940000,'AAA'],[900000,'AA'],[800000,'A'],[750000,'BBB'],[700000,'BB'],[600000,'B'],[500000,'C'],[0,'D']];
...
async function normalize(envelope,selected){
  ...
  if(!object(p)||p.handle!==selected.handle||!['intl','jp'].includes(p.region)||(selected.region!=='auto'&&p.region!==selected.region)||!text(p.displayName,200)||!p.displayName||!integer(p.createdAt)||!integer(p.updatedAt)||p.updatedAt<p.createdAt)fail();
  ...
  return {data,adapterVersion:envelope.adapterVersion,diagnostics,coverage:{...c},createdAt:p.createdAt,updatedAt:p.updatedAt,profileRating:p.rating??null};
}
async function enrichRatings(existing,incoming){
  ...
  if(existing.player.provider!=='maishift'||existing.player.key!==incoming.player.key)return existing;
  const time=core.offer(incoming).capturedAt;if(time!==core.offer(existing).capturedAt)return existing;
  const observed=core.current(incoming).pbs,eligible=new Set(),incomingIDs=new Map(Object.entries(incoming.records).map(([id,row])=>[row,id]));
  for(const c of Object.values(existing.captures))if(c.sourceKind==='maishift-public-pbs'&&c.sourceID===existing.player.key&&c.capturedAt===time&&c.playIDs.length===0&&c.sessionID===''&&c.historyCoverage==='none')for(const id of c.snapshotIDs)eligible.add(id);
  const result=structuredClone(existing),snapshotIDs=new Map();let changed=false;
  for(const sid of eligible){
    const snapshot=existing.snapshots[sid];if(snapshot.capturedAt!==time||snapshot.phase!=='after'||snapshot.complete)continue;
    const updated=structuredClone(snapshot);let filled=false;
    for(const [cid,ref]of Object.entries(snapshot.pbs)){
      const old=existing.records[ref],row=observed.get(cid);
      if(old.rate!==null||row?.rate==null||core.canonical({...old,rate:row.rate})!==core.canonical(row)||core.canonical(existing.charts[cid])!==core.canonical(incoming.charts[cid]))continue;
      const id=incomingIDs.get(row);result.records[id]=row;updated.pbs[cid]=id;filled=true;
    }
    if(filled){const id=await core.digest(updated);delete result.snapshots[sid];result.snapshots[id]=updated;snapshotIDs.set(sid,id);changed=true;}
  }
  if(!changed)return existing;
  result.captures={};for(const capture of Object.values(existing.captures)){const updated={...capture,snapshotIDs:capture.snapshotIDs.map(id=>snapshotIDs.get(id)||id)};result.captures[await core.digest(updated)]=updated;}
  const used=new Set([...Object.values(result.plays),...Object.values(result.snapshots).flatMap(s=>Object.values(s.pbs))]);
  for(const id of Object.keys(result.records))if(!used.has(id))delete result.records[id];
  const {revision,...body}=result;result.revision=await core.digest(body);return core.validate(result);
}
...
globalThis.maimaiPlayerMaishift=Object.freeze({location,regionFromURL,normalize,matchChart,enrichRatings,grade});
})();
