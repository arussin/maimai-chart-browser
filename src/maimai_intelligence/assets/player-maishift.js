...
async function normalize(envelope,selected){
  ...
  const data={format:'maimai-player-data',schemaVersion:1,player,charts:{},records:{},plays:{},snapshots:{},captures:{}},pbs={};
  for(const row of envelope.records){
    ...
    // Provider track IDs are not SEGA in-game IDs or Party catalog IDs. All
    // remain unmatched until an independently reviewed crosswalk is shipped.
    ...
    const id=await core.digest(record);data.records[id]=record;pbs[chartID]=id;
  }
...
globalThis.maimaiPlayerMaishift=Object.freeze({location,normalize});
})();
