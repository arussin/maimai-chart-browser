...
function validateSource(source,playerKey){
  ...
  if(!source||source.schemaVersion!==1||source.type!=='report'||source.adapterVersion!==1||source.playerKey!==playerKey||source.autoRefresh!==true||!Number.isSafeInteger(source.generation)||source.generation<0)throw new Error('Invalid saved source connection.');
...
globalThis.maimaiPlayerSources=Object.freeze({capabilities,reportURL,validateSource,connection,readReport,AUTO_INTERVAL,MANUAL_INTERVAL,retryAfter});
})();
